...
public class ChartHelper {
    ...
    public ChartHelper setCollectFrequency(int f){
        this.collectFrequency=f;
        return this;
    }

    public ChartHelper setCollectPoint(int p){
        this.collectPoint=p;
        return this;
    }
    ...
    public void setUnit(String unit){
        ...
        description.setText("X轴:ms Y轴:"+unit);
...
//        leftAxis.setAxisMinimum((float) dMin);
    ...
    public void showWave(float[] bytes){
        ...
        float interval=1000.0f/5120;
        StringBuilder vd= new StringBuilder();
        for (int i=0;i<2048;i++){
            bVals.add(new Entry(i*interval,bytes[i]));
            vd.append(",").append(bytes[i]);
        }
        Log.e("-----------------", vd.toString());
        showWave(bVals);
        setAxis(unit);
